  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-slate-950/98 backdrop-blur-xl animate-in fade-in transition-all">
      ...
      <style jsx global>{`
        ...
        .scan-line {
          height: 2px;
          background: linear-gradient(90deg, transparent 0%, rgba(52,211,153,0.9) 30%, #34d399 50%, rgba(52,211,153,0.9) 70%, transparent 100%);
          box-shadow: 0 0 12px 2px rgba(52,211,153,0.6), 0 0 4px 1px #34d399;
      ...
      {/* Viewfinder - Full width, professional scanner */}
      <div className="relative flex-1 flex items-center justify-center -mt-6">
        {/* Camera feed - fills the zone */}
        <div className="relative w-full aspect-[3/4] max-h-[55vh] overflow-hidden bg-black">
          <div id="reader" className="w-full h-full scale-105" />

          {/* == OVERLAY MASKS - blackout everything outside the scan box == */}
          {/* Top mask */}
          <div className="absolute top-0 left-0 right-0 h-[12%] bg-slate-950/90 z-10" />
          {/* Bottom mask */}
          <div className="absolute bottom-0 left-0 right-0 h-[12%] bg-slate-950/90 z-10" />
          {/* Left mask */}
          <div className="absolute top-[12%] left-0 w-[8%] bottom-[12%] bg-slate-950/90 z-10" />
          {/* Right mask */}
          <div className="absolute top-[12%] right-0 w-[8%] bottom-[12%] bg-slate-950/90 z-10" />

          {/* == THE SCAN WINDOW == */}
          <div className="absolute top-[12%] left-[8%] right-[8%] bottom-[12%] z-20 pointer-events-none">
            
            {/* 4 tight corner brackets - close to the edge */}
            {/* Top-left */}
            <div className="absolute top-0 left-0">
              <div className="absolute top-0 left-0 w-10 h-[3px] bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)]" />
              <div className="absolute top-0 left-0 w-[3px] h-10 bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)]" />
            </div>
            {/* Top-right */}
            <div className="absolute top-0 right-0">
              <div className="absolute top-0 right-0 w-10 h-[3px] bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)]" />
              <div className="absolute top-0 right-0 w-[3px] h-10 bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)]" />
            </div>
            {/* Bottom-left */}
            <div className="absolute bottom-0 left-0">
              <div className="absolute bottom-0 left-0 w-10 h-[3px] bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)]" />
              <div className="absolute bottom-0 left-0 w-[3px] h-10 bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)]" />
            </div>
            {/* Bottom-right */}
            <div className="absolute bottom-0 right-0">
              <div className="absolute bottom-0 right-0 w-10 h-[3px] bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)]" />
              <div className="absolute bottom-0 right-0 w-[3px] h-10 bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)]" />
            </div>
            ...
            <div className="absolute bottom-4 left-0 right-0 flex justify-center">
              <span className="text-[10px] text-emerald-400 font-black tracking-[0.25em] uppercase bg-black/50 px-4 py-1.5 rounded-full border border-emerald-500/30 backdrop-blur-md">
                ALIGNING TARGET
              </span>
            </div>
          </div>
        ...
        {error && (
          <div className="absolute bottom-4 left-4 right-4 p-4 bg-red-500/20 border border-red-500/30 text-red-100 text-[11px] text-center rounded-2xl backdrop-blur-sm z-30">
            ⚠️ {error}
          </div>
        )}
      </div>
